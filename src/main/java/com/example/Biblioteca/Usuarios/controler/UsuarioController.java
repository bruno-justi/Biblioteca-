package com.example.Biblioteca.Usuarios.controler;
...
public class UsuarioController {
    ...
    @DeleteMapping("/DeletarTeste/{id}")
    public ResponseEntity<String> deletarUsuarioTeste(@PathVariable Long id) {
        if (usuarioService.listarUsuarioPorId(id) != null) {
            usuarioService.deletarUsuarioPorId(id);
            return ResponseEntity.ok("Usuário de teste com ID: "+ id + " deletado com sucesso.");
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("Usuário de teste com o (ID) " + id + " não foi encontrado.");
        }
    }
}
